...
class IGPOActor(PPOActor):
    def _compute_turn_end_pos(self, data: dict[str, Any]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        response_end_pos = torch.full((bs, max_turns), -1, dtype=torch.long, device=device)
        for i in range(bs):
            n = len(turn_ends_list[i])
            if n > 0:
                turn_end_pos[i, :n] = turn_ends_list[i]
                response_end_pos[i, :n] = resp_ends_list[i]
        n_turns = torch.tensor([max(len(e) - 1, 0) for e in turn_ends_list], device=device)
    ...
    def _compute_ig_rewards(self, data: dict[str, Any]) -> torch.Tensor:
        ...
        answer_tokens_list, answer_spans_list = self._prepare_answer_tokens(ground_truths, device)
        ...
        batch_ids = torch.zeros(n_seqs, max_seqlen, dtype=input_ids.dtype, device=device)
        batch_attn = torch.zeros(n_seqs, max_seqlen, dtype=torch.bool, device=device)
        batch_ans_mask = torch.zeros(n_seqs, max_seqlen, dtype=torch.bool, device=device)
        ...
        with torch.no_grad():
            ...
            per_turn_logp = (
                (logprobs * batch_ans_mask.float()).sum(dim=-1)
            ...
            if valid_ig.numel() > 0:
                ...
                mu_ig, sigma_ig = torch.tensor(0.0, device=device), torch.tensor(1.0, device=device)
            ig_norm = torch.where(
                valid_mask,
            ...
            # shape: (B, seqlen)
            ig_token_rewards = torch.zeros(bs, seqlen, dtype=torch.float32, device=device)
            for i in range(bs):
