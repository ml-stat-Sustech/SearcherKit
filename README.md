...
If those variables are absent the CLI falls back to a deterministic `EchoLLM` that simply mirrors observations—useful for dry runs without network access.
...
- `--llm` chooses `auto` (env-configured) or `echo` (offline fallback).
