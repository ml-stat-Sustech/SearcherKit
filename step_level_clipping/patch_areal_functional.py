...
def _compute_step_level_ratio_and_advantages(
    ...
    starts = mask.clone()
    if log_ratio.ndim == 1:
        # In packed 1D tensors, the last valid token of one sequence can be
        # adjacent to the first valid token of the next sequence. Treat each
        # cu_seqlens boundary as a new step so those valid-token runs do not merge.
        starts[1:] &= ~mask[:-1]
...
def ppo_actor_loss_fn(
    ...
    Args:
        ...
        importance_sampling_level: Level at which to compute importance sampling ratios.
            ...
            - 'step': Step-level geometric mean over contiguous loss_mask spans
    ...
    if importance_sampling_level == "sequence":
        ...
        # Step-level IS: contiguous valid-token spans in loss_mask share one ratio.
        log_ratio = logprobs - proximal_logprobs
