...
`train/slime/train_dist_slime.yaml` should point both Elasticsearch sources to the service machine:
...
`train/slime/train_dist_slime.sh` defaults to the local 4+4 GPU split:
...
This tests `train/slime/train_dist_slime.yaml` without launching Slime training. It builds the configured eval search tool and performs one search through Elasticsearch, embedding, and summary.
...
async def main():
    raw = OmegaConf.load("train/slime/train_dist_slime.yaml")
...
ADVANTAGE_ESTIMATOR=grpo \
bash train/slime/train_dist_slime.sh \
  --num-rollout 1 \
  --save-interval 1 \
...
ADVANTAGE_ESTIMATOR=grpo \
bash train/slime/train_dist_slime.sh 2>&1 | tee logs/slime/run_logs/grpo_${RUN_TS}.log
```
...
ADVANTAGE_ESTIMATOR=igpo \
bash train/slime/train_dist_slime.sh 2>&1 | tee logs/slime/run_logs/igpo_${RUN_TS}.log
```
...
rg -n "10\\.32\\.33\\.135|10\\.32\\.34\\.135|10\\.32\\.29\\.237|embedding_max_concurrency|summary_max_concurrency" \
  train/slime/train_dist_slime.yaml
```
...
rg -n "ACTOR_NUM_GPUS_PER_NODE|ROLLOUT_NUM_GPUS|ROLLOUT_NUM_GPUS_PER_ENGINE|ROLLOUT_BATCH_SIZE|NUM_STEPS_PER_ROLLOUT|EVAL_INTERVAL|ADVANTAGE_ESTIMATOR" \
  train/slime/train_dist_slime.sh
```
...
`embedding OOM or high latency`: lower `embedding_max_concurrency` in `train/slime/train_dist_slime.yaml`, for example from `128` to `64`.
