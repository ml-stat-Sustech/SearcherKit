...
from searchagent.training.slime.client import SlimeSGLangClient
...
async def _score_igpo_answer_logprobs(
    ...
    model_name = _arg_value(args, "searchagent_igpo_model_name") or _arg_value(
        args,
        "searchagent_rollout_model_name",
        "default",
    )
...
async def generate_searchagent(
    ...
    try:
        ...
        logger.warning("SearchAgent rollout timed out after %.1fs", timeout_seconds)
    ...
    reward, reward_metadata = _score_history(
        ...
        truncation_penalty=float(_arg_value(args, "searchagent_truncation_penalty", -1.0)),
        format_error=format_error,
        repeated_query=repeated_query,
        too_many_tool_call=too_many_tool_call,
        truncated=truncated,
...
def searchagent_rollout_log(
    ...
    """Add SearchAgent trajectory diagnostics and keep slime's default rollout logging."""

    flat_samples = _flatten_samples(samples)
    metrics = _searchagent_metadata_metrics(flat_samples)
    if not metrics:
        return False

    log_dict = dict_add_prefix(metrics, "rollout/searchagent/")
    step = compute_rollout_step(args, rollout_id)
    log_dict["rollout/step"] = step
    logging_utils.log(args, log_dict, step_key="rollout/step")
    logger.info("searchagent rollout diagnostics %s: %s", rollout_id, log_dict)
    return False


def searchagent_eval_rollout_log(
    rollout_id: int,
    args: Any,
    data: dict[str, dict[str, Any]],
    extra_metrics: dict[str, Any] | None,
) -> bool:
    """Add SearchAgent trajectory diagnostics and keep slime's default eval logging."""

    log_dict: dict[str, float] = {}
    has_searchagent_metrics = False
    for dataset_name, dataset_data in data.items():
        samples = dataset_data.get("samples")
        if not samples:
            continue
        metrics = _searchagent_metadata_metrics(samples)
        if metrics:
            has_searchagent_metrics = True
            log_dict |= dict_add_prefix(metrics, f"eval/{dataset_name}/searchagent/")
    if not has_searchagent_metrics:
        return False

    step = compute_rollout_step(args, rollout_id)
    log_dict["eval/step"] = step
    logging_utils.log(args, log_dict, step_key="eval/step")
    logger.info("searchagent eval diagnostics %s: %s", rollout_id, log_dict)
    return False


def _searchagent_metadata_metrics(samples: list[Sample]) -> dict[str, float]:
    metric_keys = (
        "format_score",
        "search_score",
        "outcome_score",
