...
from searchagent.training.slime.args import (
    add_searchagent_slime_arguments,
    add_searchagent_slime_fallback_arguments,
)


def _build_fallback_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m searchagent.training.slime.train_async",
        description=(
            "SearchAgent slime async training entry. Full slime/Megatron/SGLang "
            "dependencies are required to parse and run the complete training CLI."
        ),
    )
    add_searchagent_slime_fallback_arguments(parser)
    parser.add_argument(
