...
async def _generate_rollout_async(args: Any, rollout_id: int, data_buffer: Any) -> RolloutFnTrainOutput:
    ...
    logger.info(
        "searchagent fully-async rollout %d: target=%d queue_warm=%d",
        rollout_id,
        target,
        worker.queue_size(),
    )
    while len(collected) < target:
        ...
        if now - last_log > 30:
            logger.info(
                "searchagent fully-async rollout %d: collected %d/%d, queue=%d, elapsed=%.1fs",
                rollout_id,
                len(collected),
                target,
                worker.queue_size(),
    ...
    if rollout_count < min_rollouts:
        logger.warning(
            "searchagent fully-async rollout %d returned %d rollout ids for %d groups; expected at least %d",
            rollout_id,
            rollout_count,
            len(data),
            min_rollouts,
    ...
    logger.info(
        "searchagent fully-async rollout %d: done in %.1fs, queue_left=%d, metrics=%s",
        rollout_id,
        time.time() - started,
        worker.queue_size(),
