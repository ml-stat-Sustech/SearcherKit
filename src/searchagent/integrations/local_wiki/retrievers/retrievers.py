...
class BaseRetriever(abc.ABC):
    ...
    def __init__(self, es_client: Elasticsearch, index_name: str):
        if not es_client.ping():
            raise ConnectionError("无法连接�?Elasticsearch 客户端�?)
        self.es = es_client
    ...
    def _format_results(self, hits: List[Dict[str, Any]], with_highlighted_snippet=False) -> List[Dict[str, Any]]:
        """一个辅助函数，用于�?Elasticsearch 的原始命中结果格式化为更简洁的列表�?""
        formatted_results = []
        for hit in hits:
            source = hit["_source"]
...
class BM25Retriever(BaseRetriever):
    ...
    def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        # 使用 self.search_fields 来动态指定搜索字�?
        es_query = {
            "size": top_k,
            "query": {
                "multi_match": {
                    "query": query,
                    "fields": self.search_fields, # <-- 修改�?
                    "type": "best_fields"
                }
            }
        }
...
#     def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
#         vector = self.encoder.encode(query).tolist()
#         # 对于混合搜索，k 的值需要比 top_k 大，以便�?RRF 提供足够的候选文�?
#         # num_candidates 可以根据需求调整，一般是 top_k 的几�?
...
#                 "query_vector": vector,
#                 "k": num_candidates,
#                 "num_candidates": num_candidates * 2  # num_candidates 至少�?k 的�?
#             },
#             "rank": {
#                 "rrf": {}  # 使用 Reciprocal Rank Fusion (RRF) 来融�?BM25 �?k-NN 的结�?
#             }
#         }
#         try:
#             response = self.es.search(index=self.index_name, body=es_query)
...
#     def batch_search(self, queries: List[str], top_k: int = 5) -> List[List[Dict[str, Any]]]:
#         """
#         由于 _msearch API 不支�?RRF 混合查询�?
#         我们回退到循环调用单�?search 方法来实现批量处理�?
#         """
#         # 注意：向量编码可以保持批量处理以提高效率，但这里为了逻辑简单，
#         # �?search 方法自己处理编码�?
#         # 如果追求极致性能，可以先批量编码，然后传�?search 方法�?
#         all_results = []
#         for query in queries:
#             # 直接复用已经可以工作�?single search 方法
...
class HybridRetriever(BaseRetriever):
    """
    使用混合方法进行检索�?
    由于 Elasticsearch �?RRF 是付费功能，此类在客户端手动实现 RRF 融合�?
    """
    def __init__(self, es_client: Elasticsearch, index_name: str, encoder: BaseEncoder):
        super().__init__(es_client, index_name)
        self.encoder = encoder
    ...
    def search(self, query: str, top_k: int = 5, k_rrf: int = 60) -> List[Dict[str, Any]]:
        # 1. 执行 BM25 稀疏检�?
        # 为了�?RRF 提供足够的候选文档，我们检索比 top_k 更多的结�?
        num_candidates = top_k * 10
        bm25_query = {
            "size": num_candidates,
            "query": {"multi_match": {"query": query, "fields": ["title", "text"]}}
        ...
        # 记录 Dense 排名并更�?hit 对象（dense 结果通常更相关）
...
def build_retriever(
    ...
    if retriever_class is None:
        raise ValueError(f"未知�?retriever 类型: '{retriever_type}'. 可�? 'bm25', 'dense', 'hybrid'.")

    if retriever_class == BM25Retriever:
        return BM25Retriever(es_client=es_client, index_name=index_name)
    else:
        if not encoder:
            raise ValueError(f"{retriever_class.__name__} 需要一�?encoder 实例�?)
...
if __name__ == '__main__':
    ...
    print(f"\n將使�?{len(queries_to_search)} 个查询进行批量搜�?..\n")
            ...
            for i, query in enumerate(queries_to_search):
                ...
                if not results_for_query:
                    ...
                    for j, res in enumerate(results_for_query):
                        ...
                        # 我们可以安全地访�?links，因为它总会存在
                        # print(f"     Links count: {len(res['links'])}") 
        
        except Exception as e:
            print(f"执行 {r_type.upper()} Retriever 时出�? {e}")
