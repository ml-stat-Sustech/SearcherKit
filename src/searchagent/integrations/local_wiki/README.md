# SearchAgent 本地 Wiki 检索管�?

> 建议在仓库根目录下执�?`cd src/local_wiki` 后再运行本文档中的脚本与命令，除非特殊说明�?

SearchAgent 提供了一套从维基百科原始 dump 构建本地知识库、再到为 Agent 暴露检�?访问工具的完整流水线。核心组件包括：
- `eswiki/`：解�?`enwiki-*.xml.bz2`，按需生成文本与超链，写入 Elasticsearch（支持纯 BM25、稠密向量以�?Hybrid 检索）�?
- `retrievers/`：封�?BM25、Dense、Hybrid 三类检索器，支�?`SentenceTransformer` 模型（E5、Qwen 等）的自动适配�?
- `tools/`：面�?LLM Agent 的工具接口，提供 `SearchLocalWiki`（检索标题）�?`VisitForLocalWiki`（按标题拉取正文+可行动链接）�?

## 功能亮点
- **可定制的索引构建**：`wiki2index_links.py` 支持 CPU/GPU 双批次、可�?Prompt、按需截断文本、自动创建索引映射�?
- **多种检索模�?*：BM25 稀疏检索、SentenceTransformer 稠密检索、客户端 RRF 融合�?Hybrid 检索随需切换�?
- **Agent 友好的工具协�?*：输出格式强调「下一步行动」，便于连接到自�?Agent�?
- **多模型适配**：针�?E5、Qwen/BGE 等模型内置不同前缀/参数处理；支�?`flash-attention`、多 GPU�?
...
# 然后进入 local_wiki 目录
cd src/local_wiki
# 如需 Qwen 大模型或 flash-attn，请参�?docs/install.md
```
...
### 2. 启动 Elasticsearch
使用 Docker 快速启动单节点并挂载数据卷（可参�?`scripts/docker_luncher.sh`）：
```bash
...
### 3. 构建 Wiki 索引
1. 下载 `enwiki-*-pages-articles-multistream.xml.bz2`（官�?dump 或镜像站）�?
2. 按需选择模型，并确认嵌入维度、Prompt 策略（详�?`eswiki/prompt.py`）�?
3. 运行索引脚本（以下示例启用向量索引）�?
```bash
...
脚本会自动：
- 创建索引（包含文本字段、可�?`dense_vector`）�?
- 解析维基文本为正�?+ 链接列表（过滤冗余板块）�?
- 分批送入 `SentenceTransformer`，可通过 `--dense-vector` 控制是否生成向量�?

如果需要在 CPU 侧加速解析，可改�?`eswiki/wiki2index_links_multicpu.py`。`scripts/wiki2index.sh` 提供了多种模型配置范例，可作为模板�?
...
```
该脚本会初始化检索器并触�?`SearchLocalWiki`/`VisitForLocalWiki` 的示例调用，检查模型加载和管道连通性�?
...
- 两个工具均实现了统一�?`BaseTool` 协议，可直接挂载到外�?Agent 框架�?

若希望在自定�?Python 代码中调用底层检索器，可参考：
```python
...
- `docs/model_download.sh`：批量下�?Hugging Face/ModelScope 模型的脚本模板�?
- `eswiki/cache/`：历史版本或纯文本索引脚本，可用于参考或备份方案�?

完成索引与工具验证后，即可将 `SearchLocalWiki` / `VisitForLocalWiki` 挂载到任�?Agent 系统中，为长程任务提供本地维基知识库支撑�?
