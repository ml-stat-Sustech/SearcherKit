...
def build_actor_igpo_score_requests(
    *,
    ...
    for idx in range(len(spans) - 1):
        # AReal scores the next prompt state p_{k+1}; for SearcherKit merged
...
def _build_score_rollout_data(
    *,
    ...
    micro_batch_size = max(1, int(getattr(args, "searcherkit_igpo_actor_score_micro_batch_size", 8)))
    ...
    microbatch_group_size = max(1, int(getattr(args, "searcherkit_igpo_actor_score_microbatch_group_size", 1)))
...
def attach_actor_side_igpo_rewards(
    *,
    ...
    scoring_args.searcherkit_igpo_actor_score_microbatch_group_size = 1
    if vpp_size > 1:
        scoring_args.searcherkit_igpo_actor_score_microbatch_group_size = int(
