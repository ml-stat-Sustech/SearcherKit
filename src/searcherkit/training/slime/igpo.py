...
def _advantage_estimator(args: Any) -> str:
    ...
    if value not in {"grpo", "igpo"}:
        raise ValueError(f"Unsupported SearcherKit advantage estimator: {value}")
    return value
...
def _igpo_advantages(args: Any, rollout_data: dict[str, Any]) -> None:
    ...
    ig_reward_coef = float(getattr(args, "searcherkit_igpo_reward_coef", 1.0))
    outcome_reward_coef = float(getattr(args, "searcherkit_igpo_outcome_reward_coef", 1.0))
