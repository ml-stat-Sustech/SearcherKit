...
from searcherkit.runtime.interactive_selection import apply_active_source
from searcherkit.interfaces.tui.chat.chat_history import ChatHistory
from searcherkit.interfaces.tui.runtime.provider_errors import (
    _friendly_searcherkit_error_message,
    _openai_provider_error_types,
    friendly_provider_error_message,
)
from searcherkit.runtime.interactive_selection import active_model_label, apply_active_model
...
class QueryController:
    ...
    async def run_query(self, query: str) -> None:
        ...
        try:
            ...
            raise
        except (SearcherKitError, OSError, TimeoutError) as exc:
            self._chat_history.append_run_error(
                "Run Error", _friendly_searcherkit_error_message(exc)
            )
