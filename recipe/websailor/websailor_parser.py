...
class WebSailorParser(QwenParser):
    ...
    Differences from ``QwenParser``:
    - Keep the system prompt clean instead of injecting tool specs there.
    - Prepend the first user query with a WebSailor-style tool-and-format prompt.
    - Preserve assistant ``<think>...</think>`` content in history by default.
    """
